    ...
    if count[key]>0:
      ...
      for i in range(len(bwt)):
        if bwt[i] == key:
          counter += 1
        lst[i + 1] = counter
      occ_count_before[key] = lst

  sum_so_far = 0
  for key in keys:
    if count[key]>0:
      sum_so_far += count[key]
      starts[key] = sum_so_far - count[key]

  # print('starts', starts)
  # print('occ count before', occ_count_before)
...
def CountOccurrences(pattern, bwt, starts, occ_counts_before):
  ...
  while top <= bottom:
    if pattern:
      symbol = pattern[-1]
      pattern = pattern[:-1]
      # print('symbol',symbol, 'top', top, 'bottom', bottom, 'temp_count', temp_count)
