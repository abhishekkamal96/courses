    ...
    while not q.empty():
        ...
        for id in graph.get_ids(current_node):
            ...
            if float('inf') == dist[curr_edge.v] and curr_edge.capacity > 0:
                dist[curr_edge.v] = dist[current_node] + 1
                parent[curr_edge.v] = (current_node, id)
                q.put(curr_edge.v)
                if curr_edge.v == to:
                    while True:
                        path.insert(0, id)
                        curr_x = graph.get_edge(id).capacity
                        X = min(curr_x, X)
                        if current_node == from_:
                            break
                        current_node, id = parent[current_node]
                    is_path = True
                    return is_path, path, X
    return is_path, path, X
...
def max_flow(graph, from_, to):
    ...
    # your code goes here
    while True:
        is_path, path, X = BFS(graph, from_, to)
        if not is_path:
            return flow
        for id in path:
            graph.add_flow(id, X)
